...
export class PreguntaServiceService {
  ...
  private listaDePreguntas: Pregunta[] = [
    ...
    },
    {
      ...
      texto: '¿Necesita internet para realizar la tarea?',
      opciones: [
        { opcionId: 1, texto: 'Si', valor: 1 },
        { opcionId: 2, texto: 'No', valor: 2 },
      ],
      valorCamino: 0,
      isChangeCamino: true,
    },
  ...
  getPreguntasById(id: number): Pregunta | undefined {
    const pregunta: Pregunta | undefined = this.listaDePreguntas.filter(
      (x) => x.id == id
    )[0];
    if (!pregunta) {
      throw new Error('no se encontro la pregunta');
  ...
  modifyPregunta(pregunta: Pregunta) {
    this.listaDePreguntas = this.listaDePreguntas.filter(
      (p) => p.id != pregunta.id
    );
    this.listaDePreguntas.push(pregunta);
